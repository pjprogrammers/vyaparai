...
function AuthPageInner() {
  ...
  async function handleEmailSignup(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const err = await signUpWithEmail(email, password, displayName);
      if (err) { setError(err); setLoading(false); return; }
  ...
  async function handleGitHub() {
    ...
    try {
      ...
      if (err) { setError(err); setLoading(false); return; }
      setSetupOpen(true);
    } catch { setError("GitHub sign-in failed"); } finally { setLoading(false); }
  }
  ...
  async function completeSetup() {
    if (!user) return;
    await setupBusinessAction({ uid: user.uid, email: user.email ?? email, displayName: user.displayName ?? displayName, business: biz });
    ...
    <main className="relative flex min-h-screen items-center justify-center bg-[#0a0a0a] px-6 overflow-hidden">
      ...
      >
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8">
          ...
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-400" role="alert">
              {error}
            </motion.div>
          )}

          {resetSent && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-400" role="status">
