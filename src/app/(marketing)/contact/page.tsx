  ...
  return (
    <MarketingPageWrapper scene="contact">
      ...
      <section className="mx-auto max-w-4xl px-6 py-16 text-center">
        <p className="mb-3 inline-block rounded-full bg-yellow-500/20 border border-yellow-400/30 px-4 py-1 text-sm font-medium text-yellow-400">Get in Touch</p>
      ...
      <section className="mx-auto max-w-4xl px-6 py-16">
        <StaggerList className="grid grid-cols-1 gap-8 sm:grid-cols-2">
          {[
            ...
            { icon: <svg className="h-6 w-6 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>, title: "Get a Demo", desc: "See VyaparAI in action for your business type.", link: "Start Free →", href: "/auth" },
            ...
            <StaggerListItem key={c.title}>
              <AnimatedCard className="h-full hover:border-yellow-500/30">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-yellow-500/20">{c.icon}</div>
                <h2 className="text-lg font-semibold text-white">{c.title}</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-400">{c.desc}</p>
                <a href={c.href} className="mt-4 inline-block text-sm font-semibold text-yellow-400 hover:underline">{c.link}</a>
              </AnimatedCard>
            </StaggerListItem>
          ))}
        </StaggerList>
